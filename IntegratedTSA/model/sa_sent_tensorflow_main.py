...
class Sa_Sent_Tensorflow(object):
    def __init__(self):
        ...
        self.transition = tf.get_variable(name='transition', shape=[2, 2], trainable=True)

    def my_model(self, inputs, masks, labels, length, mode):
        """
        The SA-Sent-tensorflow model, mainly three BiLSTMs + CRF.
        :param intpus: a batch of list of ids.
        :param masks: a batch of list of masks.
        ...
        with tf.variable_scope('lstm_part', reuse=tf.AUTO_REUSE) as lstm_part:
            ...
            for i in range(self.params.get('layer_num')):
                ...
                if self.params.get('layer_norm'):
                    ...
                    cell_fw = tf.contrib.rnn.DropoutWrapper(cell_fw, output_keep_prob=1-self.params.get('dropout_rate'))
                    cell_bw = tf.contrib.rnn.DropoutWrapper(cell_bw, output_keep_prob=1-self.params.get('dropout_rate'))
                (lstm_output_fw, lstm_output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                    cell_fw, cell_bw, inputs=lstm_output, sequence_length=length, dtype=tf.float32
                )
                lstm_output = tf.concat([lstm_output_fw, lstm_output_bw], axis=-1)

            lstm_output = input_embedding + tf.layers.dense(inputs=lstm_output, units=self.params.get('word_dimension'))

        # CRF layer
        with tf.variable_scope('crf_layer',reuse=tf.AUTO_REUSE) as crf_layer_layer:
            crf_layer_ = crf_layer(inputs=lstm_output, sequence_lengths=length, transition_prob=self.transition)
            crf_output = crf_layer.crf_output_prob()[:, :, -1]  # The size should be batch_size * seq_len

            # expand crf_output's shape to batch_size * 1 * seq_len for batch matrix multipcation
            crf_ouput = tf.expand_dims(crf_output, axis=1)

            sentiment_vector = tf.squeeze(tf.einsum('aij,ajk->aik', crf_output, lstm_output))  # output shape is batch_size * embedding_dim
